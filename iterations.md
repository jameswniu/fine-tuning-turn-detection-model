...
Only two of the model directories above are in git, eot-distilbert v9 and scratch-pre, so a clone can re-derive those two rows and has to take the other three on these reports.

The pretraining curve, measured as ood_test PR-AUC on unseen real calls with the same encoder at every step and the vocabulary growing with the corpus, 1,723 then 2,841 then 16,000 pieces: 0.48 random init at 3.70M, 0.60 adding real-call augmentation at 3.99M, 0.83 adding the fifteen-minute MLM pretrain at 7.36M, 0.91 for the internet-pretrained fine-tune at 66.96M. Parameter counts come from summing the ONNX initializer elements of each export. Each step is paid for in language exposure, not architecture.

Two instrument notes from the freeze. The pick_threshold CLI can segfault at interpreter teardown (exit 139) after its result printed and threshold.json was written; the artifact verified intact on disk both times, same torch plus onnxruntime teardown class as the v6 export mitigation in train.py. And the first scratch-pre2 bench read 274 req/s at concurrency 8 where the clean re-bench of scratch-pre minutes later read 588: back-to-back trainings had heated the box, so every bench runs on the exact artifact that ships and names the box state it was taken in. The fine-tune has two such readings, 33.1 ms wall p95 with the box at a load average near 4 of its 18 cores against 57.9 ms mid-training-load, and the lighter one is what the README and the hero carry with the heavier one printed beside it.
