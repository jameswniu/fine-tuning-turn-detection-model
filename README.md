...
Three of those nine steps changed nothing about the model. v7 through v9 are the same weights; what moved was the instrument. That is the finding the loop is built to surface: the eval has to be debugged with the same rigor as the model, and the fix each time was to measure the artifact that ships, in the shape it ships.
